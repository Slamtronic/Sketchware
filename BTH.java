...
public class BTH {
    ...
    private UUID uuid, MY_UUID0 = UUID.fromString("8ce255c0-223a-11e0-ac64-0803450c9a66");
...
//==========Constructors====
    ...
    // ===== سيرفر =====
    public void startServer0(String name) {
        uuid=MY_UUID0;
        if (adapter.isDiscovering()) adapter.cancelDiscovery();
    ...
    //=≈========================
   public void startServer(String tag) {
    if(adapter.isDiscovering()) adapter.cancelDiscovery();
    ...
    new Thread(() -> {
        try {
            handler.obtainMessage(STATE_LISTENING).sendToTarget();
            serverSocket = adapter.listenUsingRfcommWithServiceRecord(tag, uuid);
